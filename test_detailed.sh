...
echo "Program ID: CWpH1VjyMfHLMXeSjMxemHFGzgK2UFcZsBpfBPPCJiny"
echo "Network: Devnet"
...
PROGRAM_INFO=$(solana program show CWpH1VjyMfHLMXeSjMxemHFGzgK2UFcZsBpfBPPCJiny 2>/dev/null || echo "Error verifying program")
...
USER_ACCOUNT_PDA=$(python3 derive_pda.py "user-account" $WALLET_HEX "CWpH1VjyMfHLMXeSjMxemHFGzgK2UFcZsBpfBPPCJiny")
...
if [[ $USER_ACCOUNT_INFO == *"Error"* ]]; then
  ...
  solana program call --program-id CWpH1VjyMfHLMXeSjMxemHFGzgK2UFcZsBpfBPPCJiny \
    createUser 10 \
...
# Derive pool PDA
POOL_PDA=$(python3 derive_pda.py "bonding-pool" $TOKEN_MINT_HEX "CWpH1VjyMfHLMXeSjMxemHFGzgK2UFcZsBpfBPPCJiny")
echo "Pool PDA: $POOL_PDA"

# Derive synthetic token mint PDA
SYNTHETIC_TOKEN_MINT_PDA=$(python3 derive_pda.py "synthetic-mint" $TOKEN_MINT_HEX "CWpH1VjyMfHLMXeSjMxemHFGzgK2UFcZsBpfBPPCJiny")
echo "Synthetic token mint PDA: $SYNTHETIC_TOKEN_MINT_PDA"

# Derive real token vault PDA
REAL_TOKEN_VAULT_PDA=$(python3 derive_pda.py "token-vault" $TOKEN_MINT_HEX "CWpH1VjyMfHLMXeSjMxemHFGzgK2UFcZsBpfBPPCJiny")
...
# Derive NFT data PDA
NFT_DATA_PDA=$(python3 derive_pda.py "nft-data" $NFT_MINT_HEX "CWpH1VjyMfHLMXeSjMxemHFGzgK2UFcZsBpfBPPCJiny")
