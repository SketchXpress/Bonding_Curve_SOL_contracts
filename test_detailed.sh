...
echo "Program ID: GgLTQpotYKSqmH55whQEqnCafngwxga1RzckdkpufFMU"
echo "Network: Devnet"
...
PROGRAM_INFO=$(solana program show GgLTQpotYKSqmH55whQEqnCafngwxga1RzckdkpufFMU 2>/dev/null || echo "Error verifying program")
...
USER_ACCOUNT_PDA=$(python3 derive_pda.py "user-account" $WALLET_HEX "GgLTQpotYKSqmH55whQEqnCafngwxga1RzckdkpufFMU")
...
if [[ $USER_ACCOUNT_INFO == *"Error"* ]]; then
  ...
  solana program call --program-id GgLTQpotYKSqmH55whQEqnCafngwxga1RzckdkpufFMU \
    createUser 10 \
...
# Derive pool PDA
POOL_PDA=$(python3 derive_pda.py "bonding-pool" $TOKEN_MINT_HEX "GgLTQpotYKSqmH55whQEqnCafngwxga1RzckdkpufFMU")
echo "Pool PDA: $POOL_PDA"

# Derive synthetic token mint PDA
SYNTHETIC_TOKEN_MINT_PDA=$(python3 derive_pda.py "synthetic-mint" $TOKEN_MINT_HEX "GgLTQpotYKSqmH55whQEqnCafngwxga1RzckdkpufFMU")
echo "Synthetic token mint PDA: $SYNTHETIC_TOKEN_MINT_PDA"

# Derive real token vault PDA
REAL_TOKEN_VAULT_PDA=$(python3 derive_pda.py "token-vault" $TOKEN_MINT_HEX "GgLTQpotYKSqmH55whQEqnCafngwxga1RzckdkpufFMU")
...
# Derive NFT data PDA
NFT_DATA_PDA=$(python3 derive_pda.py "nft-data" $NFT_MINT_HEX "GgLTQpotYKSqmH55whQEqnCafngwxga1RzckdkpufFMU")
