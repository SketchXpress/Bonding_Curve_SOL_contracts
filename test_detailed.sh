...
echo "Program ID: FCpT1hnh9JKPmCR8s1rPA2ab5mETT9TFUcbDdnXhLPdu"
echo "Network: Devnet"
...
PROGRAM_INFO=$(solana program show FCpT1hnh9JKPmCR8s1rPA2ab5mETT9TFUcbDdnXhLPdu 2>/dev/null || echo "Error verifying program")
...
USER_ACCOUNT_PDA=$(python3 derive_pda.py "user-account" $WALLET_HEX "FCpT1hnh9JKPmCR8s1rPA2ab5mETT9TFUcbDdnXhLPdu")
...
if [[ $USER_ACCOUNT_INFO == *"Error"* ]]; then
  ...
  solana program call --program-id FCpT1hnh9JKPmCR8s1rPA2ab5mETT9TFUcbDdnXhLPdu \
    createUser 10 \
...
# Derive pool PDA
POOL_PDA=$(python3 derive_pda.py "bonding-pool" $TOKEN_MINT_HEX "FCpT1hnh9JKPmCR8s1rPA2ab5mETT9TFUcbDdnXhLPdu")
echo "Pool PDA: $POOL_PDA"

# Derive synthetic token mint PDA
SYNTHETIC_TOKEN_MINT_PDA=$(python3 derive_pda.py "synthetic-mint" $TOKEN_MINT_HEX "FCpT1hnh9JKPmCR8s1rPA2ab5mETT9TFUcbDdnXhLPdu")
echo "Synthetic token mint PDA: $SYNTHETIC_TOKEN_MINT_PDA"

# Derive real token vault PDA
REAL_TOKEN_VAULT_PDA=$(python3 derive_pda.py "token-vault" $TOKEN_MINT_HEX "FCpT1hnh9JKPmCR8s1rPA2ab5mETT9TFUcbDdnXhLPdu")
...
# Derive NFT data PDA
NFT_DATA_PDA=$(python3 derive_pda.py "nft-data" $NFT_MINT_HEX "FCpT1hnh9JKPmCR8s1rPA2ab5mETT9TFUcbDdnXhLPdu")
